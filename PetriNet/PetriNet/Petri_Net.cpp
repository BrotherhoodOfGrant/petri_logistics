...
// 	}
	...
	if (Name == "-1")
	{
		...
		// Пишем в поток номер в виде строки
		str_stream << (Number + 1);

		// Дописываем его к имени
		Name += str_stream.str();// (char*)(void*)System::Runtime::InteropServices::Marshal::StringToHGlobalAnsi(Convert::ToString(Number + 1));
	}
...
{
	// Проверить допустимость имени
	if (NameUnable(StartPoint)){
		return false;
	}
	// Проверить допустимость имени
	if (NameUnable(EndPoint)){
		return false;
	}
...
{
	memory_Input = new int[MAXP*MAXT];
}

int Petri_Net::get_value_Input(int col, int row)
{
	return memory_Input[MAXT*col + row];
}

void Petri_Net::set_value_Input(int col, int row, int value)
{
...
{
	memory_Output = new int[MAXP*MAXT];
}
int Petri_Net::get_value_Output(int col, int row)
{
	return memory_Output[MAXT*col + row];
}

void Petri_Net::set_value_Output(int col, int row, int value)
{
...
{
	memory_D = new int[MAXP*MAXT];
}

int Petri_Net::get_value_D(int col, int row)
{
	return memory_D[MAXT*col + row];
}

void Petri_Net::set_value_D(int col, int row, int value)
{
	memory_D[MAXT*col + row] = value;
}

int* Petri_Net::Get_Matrix_Input()
{
...
int* Petri_Net::Get_Matrix_Output()
{
...
bool Petri_Net::NameUnable(string Name)
{
